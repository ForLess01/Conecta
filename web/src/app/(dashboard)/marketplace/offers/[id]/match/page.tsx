...
export default async function OfferMatchPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ orderId?: string }> }) {
  ...
  return (
    <div className="mx-auto flex max-w-md flex-col items-center gap-6 py-16 text-center">
      ...
      <Card className="w-full"><CardContent className="space-y-3 pt-6">{order.reservationExpiresAt && <ReservationTimer expiresAt={order.reservationExpiresAt} />}<div className="space-y-1.5 border-t border-border pt-3 text-left text-sm"><Row label="Precio acordado" value={`${formatSoles(item.unitPrice)} / ${item.unit}`} /><Row label="Cantidad reservada" value={`${item.quantity.toLocaleString("es-PE")} ${item.unit}`} /><Row label="Subtotal" value={formatSoles(item.unitPrice * item.quantity)} /></div></CardContent></Card>
